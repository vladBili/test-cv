...
logging.basicConfig(
    filename="test.log",   # файл для логів
...
#.D3 product (product_id) AND invoices (prodocuct_id)
...
df_customers.to_csv("sources/sql/customers.csv", index=False)
df_products.to_csv("sources/sql/products.csv", index=False)
df_invoices.to_csv("sources/sql/invoices.csv", index=False)
df_purchases.to_csv("sources/sql/purchases.csv", index=False)